...
func (model userModel) FindByID(id string) (data UserEntity, err error) {
	query :=
		`SELECT u."id", u."username", u."email", u."name", u."password", u."role_id", r."name",
		u."odo_user_id", u."profile_picture", f."path", u."is_active", u."created_at", u."updated_at",
		u."deleted_at"
		FROM "users" u
	...
	err = model.DB.QueryRow(query, id).Scan(
		...
		&data.OdoUserID, &data.ProfilePicture, &data.ProfilePicturePath, &data.IsActive, &data.CreatedAt, &data.UpdatedAt,
		&data.DeletedAt,
	)
